...
class AtomisticReverseMonteCarlo(ModifierInterface):
    ...
    def modify(self, data: DataCollection, frame: int, data_cache: DataCollection, **kwargs):
        ...
        pairs = list(product(range(len(data_cache.attributes["wc_trajectory"][0])), repeat=2))
        ...
        labels = [
            f'{self.get_type_name(data, unique_types[i])}-{self.get_type_name(data, unique_types[j])}' for i, j in pairs]
